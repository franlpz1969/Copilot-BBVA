...
const AITransferSummary = ({ transfers }) => {
    ...
    return (
        <div className="bg-light-blue p-4 rounded-lg border border-blue-200 shadow-lg mb-6">
            <div className="flex items-start space-x-3">
                <SparklesIcon className="h-6 w-6 text-bbva-accent flex-shrink-0" />
                <div>
                    <h4 className="font-bold text-bbva-blue">Resumen del Asistente IA</h4>
...
const FiltersSidebar = ({ filters, setFilters, responsibleParties }) => {
    ...
    return (
        <div className="bg-white p-6 rounded-lg shadow-lg h-full">
            <div className="flex justify-between items-center mb-4"><h3 className="text-lg font-semibold text-gray-800">Filtros</h3><FilterIcon className="h-5 w-5 text-gray-500" /></div>
            <div className="space-y-6">
                <div><label className="block text-sm font-medium text-gray-700 mb-2">Riesgo</label><div className="space-y-2"><div className="flex items-center"><input type="checkbox" value="Bajo riesgo" onChange={handleRiskChange} checked={filters.risk.includes('Bajo riesgo')} className="h-4 w-4 rounded accent-bbva-accent"/> <span className="ml-2 text-sm">Bajo riesgo</span></div><div className="flex items-center"><input type="checkbox" value="Medio riesgo" onChange={handleRiskChange} checked={filters.risk.includes('Medio riesgo')} className="h-4 w-4 rounded accent-bbva-accent"/> <span className="ml-2 text-sm">Medio riesgo</span></div><div className="flex items-center"><input type="checkbox" value="Alto riesgo" onChange={handleRiskChange} checked={filters.risk.includes('Alto riesgo')} className="h-4 w-4 rounded accent-bbva-accent"/> <span className="ml-2 text-sm">Alto riesgo</span></div></div></div>
                <div><label htmlFor="amount" className="block text-sm font-medium text-gray-700">Monto Máximo</label><input type="range" id="amount" name="amount" min="0" max="160000" step="1000" value={filters.amount} onChange={e => setFilters(f => ({...f, amount: Number(e.target.value)}))} className="mt-1 block w-full accent-bbva-accent" /><div className="flex justify-between text-xs text-gray-500"><span>€0</span><span>€{filters.amount.toLocaleString()}</span></div></div>
                <div><label htmlFor="responsible" className="block text-sm font-medium text-gray-700">Responsable</label><select id="responsible" value={filters.responsible} onChange={e => setFilters(f => ({...f, responsible: e.target.value}))} className="mt-1 w-full p-2 border rounded-md text-sm focus:ring-bbva-accent focus:border-bbva-accent">{responsibleParties.map(r => <option key={r} value={r}>{r}</option>)}</select></div>
                <div className="border-t pt-4 space-y-2"><button onClick={clearFilters} className="w-full text-gray-600 py-2 px-4 rounded-lg hover:bg-gray-100 border">Limpiar Filtros</button></div>
            </div>
        </div>
    );
};
...
const TransferCard: React.FC<{ transfer: Transfer }> = ({ transfer }) => (
    <div className={`p-4 rounded-lg shadow-lg ${riskStyles[transfer.status === 'completed' ? 'completed' : transfer.risk || 'Bajo riesgo']}`}>
        <div className="flex justify-between items-start">
            <div>
                <p className="font-bold text-lg text-bbva-blue">{transfer.amount.toLocaleString('es-ES', { style: 'currency', currency: 'EUR' })}</p>
                <p className="text-sm font-medium text-gray-700">{transfer.provider}</p>
            </div>
            <div className="text-right">
                <p className="text-sm text-gray-500">Resp: {transfer.responsible}</p>
                <p className="text-sm text-gray-500">{transfer.date}</p>
                {transfer.status === 'completed' && <span className="text-xs font-bold text-gray-500">FIRMADO</span>}
            </div>
        </div>
    </div>
);
            ...
            <div className="lg:col-span-5 space-y-6">
                 ...
                 <div className="bg-white p-6 rounded-lg shadow-lg">
                    <div className="flex justify-between items-center mb-4">
                        ...
                        <button onClick={handleSignLowRisk} disabled={pendingLowRiskCount === 0} className="bg-bbva-blue text-white font-semibold py-2 px-4 rounded-lg hover:bg-opacity-90 disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors">Firmar {pendingLowRiskCount} de bajo riesgo</button>
                    </div>
                     {showSuccess && (<div className="bg-green-100 border border-green-300 text-green-800 px-4 py-2 rounded-lg mb-4 text-center flex items-center justify-center space-x-2"><CheckCircleIcon className="h-5 w-5"/><span>{showSuccess}</span></div>)}
                    <div className="space-y-3 h-[400px] overflow-y-auto pr-2">
                        {filteredTransfers.length > 0 ? (
                            filteredTransfers.map(t => <TransferCard key={t.id} transfer={t} />)
                        ) : (<p className="text-center text-gray-500 pt-10">No hay transferencias que coincidan con los filtros.</p>)}
                    </div>
                </div>
            </div>
